...
export async function generateStaticParams() {
  const lessons = getLessonsByCourseName('claude-code')
  return lessons.map((lesson) => ({ slug: lesson.slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const lesson = getLessonBySlug('claude-code', params.slug)
  ...
  return {
    ...
    openGraph: {
      ...
      description: lesson.frontmatter.description,
      url: `/khoa-hoc/claude-code/${params.slug}`,
      siteName: SITE_NAME,
      locale: 'vi_VN',
      type: 'article',
    ...
    alternates: { canonical: `/khoa-hoc/claude-code/${params.slug}` },
  }
}

export default function ClaudeCodeLessonPage({ params }: PageProps) {
  const lesson = getLessonBySlug('claude-code', params.slug)
  if (!lesson) notFound()

  const allLessons = getLessonsByCourseName('claude-code')
  const { prev, next } = getAdjacentLessons('claude-code', params.slug)
  ...
  const jsonLd = [
    {
      ...
      mainEntityOfPage: toAbsoluteUrl(`/khoa-hoc/claude-code/${params.slug}`),
      url: toAbsoluteUrl(`/khoa-hoc/claude-code/${params.slug}`),
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: 'Khóa học', item: toAbsoluteUrl('/khoa-hoc') },
        {
          ...
          position: 3,
          name: 'Claude Code',
          item: toAbsoluteUrl('/khoa-hoc/claude-code'),
        },
        {
          '@type': 'ListItem',
          position: 4,
          name: lesson.frontmatter.title,
          item: toAbsoluteUrl(`/khoa-hoc/claude-code/${params.slug}`),
        },
      ],
    },
  ]
      ...
      <div className="border-b border-slate-200 bg-gradient-to-b from-slate-50 to-white px-4 py-8 sm:px-6 xl:px-8">
        <div className="site-frame">
          <nav className="text-sm text-slate-500">
            ...
            <span className="mx-2">/</span>
            <Link href="/khoa-hoc/claude-code" className="hover:text-blue-600">
              Claude Code
            </Link>
            <span className="mx-2">/</span>
      ...
      <div className="site-container py-10">
        <div className="flex gap-8 xl:gap-10 2xl:gap-12">
          ...
          <article className="min-w-0 flex-1">
            ...
            <LessonNav prev={prev} next={next} courseSlug="claude-code" />
