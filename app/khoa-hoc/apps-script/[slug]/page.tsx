...
export async function generateStaticParams() {
  const lessons = getLessonsByCourseName('apps-script')
  return lessons.map((lesson) => ({ slug: lesson.slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const lesson = getLessonBySlug('apps-script', params.slug)
  ...
  return {
    ...
    openGraph: {
      ...
      description: lesson.frontmatter.description,
      url: `/khoa-hoc/apps-script/${params.slug}`,
      siteName: SITE_NAME,
      locale: 'vi_VN',
      type: 'article',
    ...
    alternates: { canonical: `/khoa-hoc/apps-script/${params.slug}` },
  }
}

export default function AppsScriptLessonPage({ params }: PageProps) {
  const lesson = getLessonBySlug('apps-script', params.slug)
  if (!lesson) notFound()

  const allLessons = getLessonsByCourseName('apps-script')
  const { prev, next } = getAdjacentLessons('apps-script', params.slug)
  ...
  const jsonLd = [
    {
      ...
      mainEntityOfPage: toAbsoluteUrl(`/khoa-hoc/apps-script/${params.slug}`),
      url: toAbsoluteUrl(`/khoa-hoc/apps-script/${params.slug}`),
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: 'Khóa học', item: toAbsoluteUrl('/khoa-hoc') },
        {
          ...
          position: 3,
          name: 'Apps Script',
          item: toAbsoluteUrl('/khoa-hoc/apps-script'),
        },
        {
          '@type': 'ListItem',
          position: 4,
          name: lesson.frontmatter.title,
          item: toAbsoluteUrl(`/khoa-hoc/apps-script/${params.slug}`),
        },
      ],
    },
  ]
      ...
      <div className="border-b border-slate-200 bg-gradient-to-b from-slate-50 to-white px-4 py-8 sm:px-6 xl:px-8">
        <div className="site-frame">
          <nav className="text-sm text-slate-500">
            ...
            <span className="mx-2">/</span>
            <Link href="/khoa-hoc/apps-script" className="hover:text-blue-600">
              Apps Script
            </Link>
            <span className="mx-2">/</span>
      ...
      <div className="site-container py-10">
        <div className="flex gap-8 xl:gap-10 2xl:gap-12">
          ...
          <article className="min-w-0 flex-1">
            ...
            <LessonNav prev={prev} next={next} courseSlug="apps-script" />
