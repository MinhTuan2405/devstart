...
export async function generateStaticParams() {
  const lessons = getLessonsByCourseName('git')
  return lessons.map((lesson) => ({ slug: lesson.slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const lesson = getLessonBySlug('git', params.slug)
  ...
  return {
    ...
    alternates: { canonical: `/khoa-hoc/git/${params.slug}` },
  }
}

export default function GitLessonPage({ params }: PageProps) {
  const lesson = getLessonBySlug('git', params.slug)
  if (!lesson) notFound()

  const allLessons = getLessonsByCourseName('git')
  const { prev, next } = getAdjacentLessons('git', params.slug)
  ...
  const jsonLd = [
    {
      ...
      mainEntityOfPage: toAbsoluteUrl(`/khoa-hoc/git/${params.slug}`),
      url: toAbsoluteUrl(`/khoa-hoc/git/${params.slug}`),
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: 'Git', item: toAbsoluteUrl('/khoa-hoc/git') },
        {
          '@type': 'ListItem',
          position: 4,
          name: lesson.frontmatter.title,
          item: toAbsoluteUrl(`/khoa-hoc/git/${params.slug}`),
        },
      ],
    },
  ]
      ...
      <div className="border-b border-slate-200 bg-gradient-to-b from-slate-50 to-white px-4 py-8 sm:px-6 xl:px-8">
        <div className="site-frame">
          <nav className="text-sm text-slate-500">
            ...
            <span className="mx-2">/</span>
            <Link href="/khoa-hoc/git" className="hover:text-blue-600">Git</Link>
            <span className="mx-2">/</span>
      ...
      <div className="site-container py-10">
        <div className="flex gap-8 xl:gap-10 2xl:gap-12">
          ...
          <article className="min-w-0 flex-1">
            ...
            <LessonNav prev={prev} next={next} courseSlug="git" />
