...
export async function generateStaticParams() {
  const lessons = getLessonsByCourseName('sql')
  return lessons.map((lesson) => ({ slug: lesson.slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const lesson = getLessonBySlug('sql', params.slug)
  ...
  return {
    ...
    alternates: { canonical: `/khoa-hoc/sql/${params.slug}` },
  }
}

export default function SqlLessonPage({ params }: PageProps) {
  const lesson = getLessonBySlug('sql', params.slug)
  if (!lesson) notFound()

  const allLessons = getLessonsByCourseName('sql')
  const { prev, next } = getAdjacentLessons('sql', params.slug)
  ...
  const jsonLd = [
    {
      ...
      mainEntityOfPage: toAbsoluteUrl(`/khoa-hoc/sql/${params.slug}`),
      url: toAbsoluteUrl(`/khoa-hoc/sql/${params.slug}`),
    },
    {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: 'SQL', item: toAbsoluteUrl('/khoa-hoc/sql') },
        {
          '@type': 'ListItem',
          position: 4,
          name: lesson.frontmatter.title,
          item: toAbsoluteUrl(`/khoa-hoc/sql/${params.slug}`),
        },
      ],
    },
  ]
      ...
      <div className="border-b border-slate-200 bg-gradient-to-b from-slate-50 to-white px-4 py-8 sm:px-6 xl:px-8">
        <div className="site-frame">
          <nav className="text-sm text-slate-500">
            ...
            <span className="mx-2">/</span>
            <Link href="/khoa-hoc/sql" className="hover:text-blue-600">SQL</Link>
            <span className="mx-2">/</span>
      ...
      <div className="site-container py-10">
        <div className="flex gap-8 xl:gap-10 2xl:gap-12">
          ...
          <article className="min-w-0 flex-1">
            ...
            <LessonNav prev={prev} next={next} courseSlug="sql" />
