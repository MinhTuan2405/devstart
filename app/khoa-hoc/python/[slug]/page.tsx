...
export async function generateStaticParams() {
  const lessons = getLessonsByCourseName('python')
  return lessons.map((lesson) => ({ slug: lesson.slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const lesson = getLessonBySlug('python', params.slug)
...
export default function PythonLessonPage({ params }: PageProps) {
  ...
  const allLessons = getLessonsByCourseName('python')
  const { prev, next } = getAdjacentLessons('python', params.slug)
  ...
  const jsonLd = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: 'Python', item: toAbsoluteUrl('/khoa-hoc/python') },
        {
          '@type': 'ListItem',
          position: 4,
          name: lesson.frontmatter.title,
          item: toAbsoluteUrl(`/khoa-hoc/python/${params.slug}`),
        },
      ],
    },
  ]
    ...
    <>
      ...
      <div className="border-b border-slate-200 bg-gradient-to-b from-slate-50 to-white px-4 py-8 sm:px-6 xl:px-8">
        <div className="site-frame">
          <nav className="text-sm text-slate-500">
            ...
            <span className="mx-2">/</span>
            <Link href="/khoa-hoc/python" className="hover:text-blue-600">Python</Link>
            <span className="mx-2">/</span>
      ...
      <div className="site-container py-10">
        <div className="flex gap-8 xl:gap-10 2xl:gap-12">
          <Sidebar lessons={allLessons} courseName="Python" courseSlug="python" />
          ...
          <article className="min-w-0 flex-1">
            ...
            <LessonNav prev={prev} next={next} courseSlug="python" />
